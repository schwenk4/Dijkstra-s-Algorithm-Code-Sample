...
/* Set to 1 to remove all walls as a debugging aid */
#define GOD_MODE 0
...
/* 
 * get_n_fruits
 *   DESCRIPTION: helper to get number of fruits
 *   INPUTS: none
 *   OUTPUTS: n_fruits
...
int get_block_of_data(int x,int y){
    return find_block(x,y);
}
/* 
 * mark_maze_area
...
/*
 * store_mask
 *   DESCRIPTION:  determines which pixels are copied from the player image to the
 *   screen and stores the old values in a buffer
 *   INPUTS: x - x value of square player is on
 *           y - y value of square player is on
 *   OUTPUTS: none
 *   RETURN VALUE: none
 *   SIDE EFFECTS: Stores data overwritten by player into buffer
 */
void store_mask(int x, int y){
    masks.latest_mask = get_block_of_data(x,y);//get block data from memory
}
/*
 * undraw_player_mask
 *   DESCRIPTION:  takes the mask and the pixel values copied from the floor 
 *   and puts them back into the screen
 *   INPUTS: x - x value of square player is on
 *           y - y value of square player is on
 *           img - image to replace
 *           prev_dir - direction player came from, to find square to unmask
 *   OUTPUTS: none
 *   RETURN VALUE: none
 *   SIDE EFFECTS: Writes mask information to memory
 */
void undraw_player_mask(int x, int y, unsigned char* img, int prev_dir){
    switch (prev_dir) {
    case DIR_UP:    
        y=y-1;    
        break;
    ...
    draw_full_block(x,y,img);
}
/*
 * get_mask_block
 *   DESCRIPTION:  gets pre-mask image from buffer
 *   INPUTS: none
 *   OUTPUTS: image from buffer
 *   RETURN VALUE: image from buffer
 */
unsigned char * get_mask_block(){
    unsigned char * temp_mask = masks.previous_mask;
    masks.previous_mask = masks.latest_mask; 
    if(temp_mask == 0x00){ //if this is the first block masked/unmasked
        return masks.latest_mask;
    }
    return temp_mask;
}
/* 
